...
ESPCN: leftthomas/ESPCN — Efficient Sub-Pixel Convolutional Neural Network for real-time video super-resolution.
EVASR: symmru/EVASR_MMsys2023 — Generates 6x6 tile-based saliency weight matrices (.npy).
STVS: guotaowang/STVS — Spatio-temporal video saliency detection model.
...
System Requirement: FFmpeg built with CUDA (libvmaf_cuda) support must be installed in system PATH.

4. Directory Structure
Download the 4K raw YUV test videos from the UVG Dataset Website and ensure your directory is organized as follows:
├── live_demo.py
├── live_demo.sh
...
└── super_resolution_build/
    ...
    │   ├── epoch_4_100.pt
    │   └── epoch_8_100.pt
    └── EVASR/saliency_weight/    # Saliency weight matrices (.npy)
...
6. Run VMAF CUDA Evaluation (live_demo.sh)Ensure the script is executable before running:
